...
@section('thead')
    ...
    <th>
        <div><i class="fas fa-info"></i><span>Em(Curso)</span></div>
    </th>
    @isset(Auth::user()->cliente->id)
        <th colspan="2">
            <div><i class="fas fa-info-circle"></i><span>Avaliação</span></div>
        </th>
    @endisset
    <th colspan="2">
...
@section('tbody')
    @foreach ($encomendas as $encomenda)
        <tr>
            <td cliente_id="{{ $encomenda->cliente_id }}" email="{{ $encomenda->cliente->user->email }}">
                {{ $encomenda->cliente->user->name }}</td>
            ...
            @isset(Auth::user()->funcionario->id)
                <td class="text-align-center">
                    <button
                        class="form-check ml-4 encomendaEmCurso btn btn-sm {{ !$encomenda->atendido ? 'btn-warning' : 'btn-danger' }}"
                        value="{{ $encomenda->atendido }}" data-bs-toggle="modal" data-bs-target="#modalConfirm"
                        url="{{ route('encomenda.atender', $encomenda->id) }}">
                        {{ !$encomenda->atendido ? 'anteder' : 'cancelar' }}
                    </button>
                </td>
            @else
                <td>{{ $encomenda->atendido ? 'sim' : 'não' }}</td>
            @endisset
            @isset(Auth::user()->cliente->id)
                <td>
                    <a href="#" class="text-warning rounded btn-sm btn-avaliacao-add d-flex gap-1 align-items-center"
                        data-bs-toggle="modal" data-bs-target="#modalAvaliacao" code="" title="Adicionar\Produto"
                        url="{{ route('avaliacao.encomenda.add', $encomenda->id) }}">
                        <i class="fas fa-plus"></i>
                        <span>adicionar</span>
                    </a>
                </td>
                <td>
                    <a href="{{ route('avaliacao.encomenda.list', $encomenda->id) }}" class="text-primary rounded btn-sm btn-produto-list d-flex gap-1 align-items-center">
                        <i class="fas fa-bars"></i>
                        <span>listar</span>
                        <sup>{{ count($encomenda->avaliacoes) }}</sup>
                    </a>
                </td>
            @endisset
            <td>
                <a href="#" class="text-success rounded btn-sm btn-produto-add d-flex gap-1 align-items-center"
                    data-bs-toggle="modal" data-bs-target="#modalProdutoList" code="" title="Adicionar\Produto"
            ...
            <td>
                <a href="#" class="text-info rounded btn-sm btn-produto-list d-flex gap-1 align-items-center"
                    data-bs-toggle="modal" data-bs-target="#modalProdutoList" code="" title="Listar\Produto"
            ...
            <td>
                <a href="#" class="text-warning rounded btn-sm btn-user-tr d-flex gap-1 align-items-center"
                    data-bs-toggle="modal" data-bs-target="#modalEncomenda"
                    url="{{ route($panel . '.update', $encomenda->id) }}" method="PUT">
            ...
            <td>
                <a href="#" class="text-danger rounded btn-sm btn-user-del d-flex gap-1 align-items-center"
                    data-bs-toggle="modal" data-bs-target="#modalEncomenda"
                    url="{{ route($panel . '.destroy', $encomenda->id) }}" method="DELETE">
...
@section('modal')
    ...
    @include('components.modal.confirm', [
        'action' => '#',
        'method' => 'get',
        'message' => 'Tens a certeza que desejas atender esta encomenda?',
    ])
    <div hidden>
